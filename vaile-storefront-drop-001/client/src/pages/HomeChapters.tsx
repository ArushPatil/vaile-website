...
const shots = [
  { id: "01", title: "FRONT PROFILE", spec: "Straight leg block / natural drop over footwear", desktop: "/images/1-desktop.jpg", mobile: "/images/1-mobile.jpg", alt: "VAILE Drop 001 black duck canvas pants front full standing view" },
  { id: "02", title: "LATERAL DRAPE", spec: "Side profile showing relaxed seat and structured leg line", desktop: "/images/2-desktop.jpg", mobile: "/images/2-mobile.jpg", alt: "VAILE Drop 001 black duck canvas pants side profile view" },
  { id: "03", title: "HARDWARE & POCKETS", spec: "Copper rivets and reinforced tool pocketing", desktop: "/images/3-desktop.jpg", mobile: "/images/3-mobile.jpg", alt: "VAILE Drop 001 canvas pocket and rivet hardware" },
  { id: "04", title: "REAR YOKE & SEAT", spec: "Structured back rise with reinforced bar-tacks", desktop: "/images/4-desktop.jpg", mobile: "/images/4-mobile.jpg", alt: "VAILE Drop 001 back pocket details" },
  { id: "05", title: "SEATED BREAK", spec: "Canvas break and knee articulation under use", desktop: "/images/6-desktop.jpg", mobile: "/images/6-mobile.jpg", alt: "VAILE Drop 001 seated canvas break" },
  { id: "06", title: "STRIDE IN MOTION", spec: "Fabric drape during active movement", desktop: "/images/9-desktop.jpg", mobile: "/images/9-mobile.jpg", alt: "VAILE Drop 001 walking silhouette" },
];
  ...
  return <>
    ...
    <main className={loading ? "manual-shell is-loading chapter-shell" : "manual-shell chapter-shell"}>
      ...
      <div className="manual-stack chapter-stack">
        ...
        <section id="gallery" className="chapter chapter-gallery" data-stack-item><div className="gallery-ribbon"><header className="gallery-header"><div><span>FIELD EVIDENCE / SIX VIEWS</span><h2>Lookbook <em>/ 001</em></h2></div><div className="gallery-controls"><b>FIGURE {shot.id} / 0{shots.length}</b><div><button onClick={() => setActive((active + shots.length - 1) % shots.length)} aria-label="Previous model shot"><ChevronLeft size={18} /></button><button onClick={() => setActive((active + 1) % shots.length)} aria-label="Next model shot"><ChevronRight size={18} /></button></div></div></header><div className="gallery-stage"><AnimatePresence mode="wait"><motion.figure key={shot.id} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={{ duration: 0.28 }}><picture><source media="(max-width: 640px)" srcSet={shot.mobile} /><img src={shot.desktop} alt={shot.alt} loading="lazy" /></picture><figcaption>SPECIMEN {shot.id} / DROP 001</figcaption></motion.figure></AnimatePresence><div className="gallery-caption"><span>FIG. {shot.id}</span><h3>{shot.title}</h3><p>{shot.spec}</p><i /></div></div><div className="gallery-pills">{shots.map((item, index) => <button key={item.id} className={active === index ? "is-active" : ""} onClick={() => setActive(index)} aria-label={`View figure ${item.id}: ${item.title}`}>{item.id}</button>)}</div></div></section>

        <section className="chapter chapter-cut" data-stack-item><div className="cut-layout"><div className="cut-title"><span>02 / THE CUT</span><h2>Easy through<br />the leg. Clear<br />over a shoe.</h2><p>Enough room to move, enough structure to hold its line. The silhouette is designed to wear in, not pose in.</p></div><figure><img src={assets.front} alt="Full-length outdoor view of black duck canvas pants" /><figcaption>STRAIGHT FALL / NATURAL BREAK</figcaption></figure><blockquote>“A uniform should get better with use.”</blockquote></div></section>

        <section id="fit" className="chapter chapter-fit" data-stack-item><div className="fit-layout"><div className="fit-marker">03<br /><span>FIT</span></div><div className="fit-main"><Head index="FIT REFERENCE" title="Fit, without the guesswork." note="Your preferred waist is a starting point. Ask for exact measurements before allocating." /><div className="fit-list"><article><span>PROFILE</span><b>Relaxed straight leg</b></article><article><span>WEAR TRUE</span><b>Your usual waist for an easy fit</b></article><article><span>SIZE UP</span><b>More room through seat and leg</b></article><article><span>CONFIRM</span><b>Waist / rise / thigh / hem / inseam via WhatsApp</b></article></div></div><figure><img src={assets.side} alt="Side profile of VAILE duck canvas trousers" /><figcaption>MEASURE BEFORE ALLOCATION</figcaption></figure></div></section>
        ...
        <section id="care" className="chapter chapter-care" data-stack-item><div className="care-layout"><div><Head index="05" title="Keep the wear. Skip the damage." note="Care reference / clean only when needed. The garment label always takes priority." /><p>Duck canvas earns character slowly. Spot clean first; wash cold only when it needs it; skip bleach and high heat.</p></div><ul><li><b>01</b><span>Spot clean first</span></li><li><b>02</b><span>Wash cold, gently</span></li><li><b>03</b><span>Hang dry</span></li><li><b>04</b><span>Iron inside out, warm</span></li></ul><aside>WEAR LOG<br />OPEN ENDED</aside></div></section>
      </div>
